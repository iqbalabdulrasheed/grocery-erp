...
export const db = {
  ...
  removeItemFromOrder: async (orderId: string, productId: string, actor: User): Promise<Order> => {
    ...
    const discountAmount = order.subtotal > 0 ? (order.subtotal - order.total) : 0;
...
// ── COMPANY SETTINGS ──────────────────────────────────────────────────────────
const COMPANY_KEY = 'company_settings';
...
export const saveCompanySettings = async (settings: CompanySettings): Promise<void> => {
  if (supabase) {
    ...
    if (data) {
      await supabase.from('company_settings').update(settings).eq('id', (data as any).id);
    } else {
...
export const saveInvoice = async (invoice: Invoice): Promise<void> => {
  if (supabase) {
    const { data } = await supabase.from('invoices').select('id').eq('id', invoice.id).single();
    if (data) {
    ...
    const list = JSON.parse(localStorage.getItem(INVOICE_KEY) || '[]') as Invoice[];
...
export const getInvoiceByOrderId = async (orderId: string): Promise<Invoice | null> => {
  ...
  const list = JSON.parse(localStorage.getItem(INVOICE_KEY) || '[]') as Invoice[];
  return list.find(i => i.order_id === orderId) || null;
};

export const buildInvoiceFromOrder = async (order: Order, customer: User): Promise<Invoice> => {
  const company = await getCompanySettings();
  const now = new Date();
  const dateStr = now.toISOString().split('T')[0];
  const timeStr = now.toTimeString().split(' ')[0];
  const paddedNum = String(company.next_invoice_number).padStart(6, '0');
  const invoiceNumber = `${company.invoice_prefix}-${now.getFullYear()}-${paddedNum}`;

  // Increment counter
  await saveCompanySettings({ ...company, next_invoice_number: company.next_invoice_number + 1 });

  const VAT_RATE = 0.15;
  const lines: InvoiceLine[] = order.items.map(item => {
    const lineDiscount = 0;
    const taxable = Number((item.unit_price * item.qty - lineDiscount).toFixed(2));
    const vatAmt = Number((taxable * VAT_RATE).toFixed(2));
    return {
      product_id: item.product_id,
      ...
      vat_rate: 15,
      vat_amount: vatAmt,
      line_total: Number((taxable + vatAmt).toFixed(2)),
    };
  });

  const subtotal = Number(order.subtotal.toFixed(2));
  const discount = Number((order.discount + (order.manual_discount_amt || 0)).toFixed(2));
  const taxable = Number((subtotal - discount).toFixed(2));
  const vatAmount = Number((taxable * VAT_RATE).toFixed(2));
  const grandTotal = Number((taxable + vatAmount).toFixed(2));

  // Simple ZATCA Phase-1 QR: base64 of TLV fields
  const tlv = (tag: number, val: string) => {
    const enc = new TextEncoder().encode(val);
    return [tag, enc.length, ...enc];
  };
  const qrBytes = [
    ...tlv(1, company.name_en || 'Seller'),
    ...tlv(2, company.vat_number || ''),
    ...tlv(3, now.toISOString()),
    ...tlv(4, grandTotal.toFixed(2)),
    ...tlv(5, vatAmount.toFixed(2)),
  ];
  const qrBase64 = btoa(String.fromCharCode(...qrBytes));

  const uuid = `${Date.now().toString(16)}-${Math.random().toString(16).slice(2,6)}-4${Math.random().toString(16).slice(2,5)}-${Math.random().toString(16).slice(2,6)}-${Math.random().toString(16).slice(2,14)}`;
  ...
  return {
    ...
    invoice_time: timeStr,
    invoice_type: customer.is_b2b ? 'tax' : 'simplified',
    order_id: order.id,
    seller_name_en: company.name_en,
    ...
    payment_method: order.cod_tracking ? 'Cash on Delivery' : 'Credit',
    payment_terms: order.cod_tracking ? 'Due on delivery' : 'Net 30',
    notes: `Order Reference: ${order.id}`,
    qr_code: qrBase64,
    status: 'issued',
  };
};
